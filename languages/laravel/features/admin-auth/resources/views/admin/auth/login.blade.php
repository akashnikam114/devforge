...
<head>
    ...
    <title>Login | {{ config('devforge-ui.app_name', '__PROJECT_NAME__') }}</title>
    <link rel="manifest" href="{{ asset('pwa/manifest.json') }}">
    <script src="https://cdn.tailwindcss.com"></script>
    <style>
        :root {
            --admin-primary: {{ config('devforge-ui.primary_color', '#049C9C') }};
            --admin-secondary: {{ config('devforge-ui.secondary_color', '#037a7a') }};
        }
    ...
    <div class="flex min-h-screen">
        <div class="flex items-center justify-center w-full lg:w-[45%] px-6 sm:px-12">
            <div class="w-full max-w-sm">
                <div class="mb-10">
                    <h1 class="text-3xl font-extrabold text-gray-900">{{ config('devforge-ui.app_name', '__PROJECT_NAME__') }}</h1>
                ...
                <form method="POST" action="{{ route('admin.login') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" class="w-full px-4 py-3.5 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-4 focus:ring-teal-100 focus:border-teal-600 outline-none" placeholder="admin@example.com">
                    ...
                    <div>
                        ...
                        <input type="password" name="password" class="w-full px-4 py-3.5 border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:ring-4 focus:ring-teal-100 focus:border-teal-600 outline-none" placeholder="Password">
        ...
        <div class="hidden lg:flex w-[55%] login-gradient items-center justify-center px-16 text-white">
            <div class="max-w-lg">
                <h2 class="text-4xl font-extrabold mb-4">{{ config('devforge-ui.panel_title', 'Admin Panel') }}</h2>
                <p class="text-white/85 text-lg">{{ config('devforge-ui.panel_description', 'Manage users, settings, reports, and application operations from one place.') }}</p>
            </div>
        </div>
    </div>
</body>
